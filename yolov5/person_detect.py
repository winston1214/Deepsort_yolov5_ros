...
def detect(save_img=False):
    ...
    # rail_pts = np.array([[988, 357], [1107, 357], [1643, 1073], [577, 1073]], np.int32) # 광각
    # rail_pts = np.array([[732, 291], [943, 291], [1284, 1073], [100, 1073]], np.int32) #광각 X
    # Run inference
    t0 = time.time()
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            img_ls.append(im0)
            # cv2.putText(im0,'{}'.format(num_frame),(130, 180), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0, 255, 255), 3)
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if label == 'person':
                            ...
                            idx=len(input_data)-1
                            
                            # if len(img_ls) >= 2 and names[int(cls)] == 'person':
                            #     try:
                            #         ang,mag = optical_flow(xyxy,img_ls[-2],img_ls[-1])
                            #         if ang<0: ang+=360
                            #         center_x = (x1+x2)//2
                            #         center_y = (y1+y2)//2
                            #         new_x = center_x + mag * np.cos(np.radians(ang))
                            #         new_y = center_y + mag * np.sin(np.radians(ang))
                            #         cv2.arrowedLine(im0,(int(center_x),int(center_y)),(int(new_x),int(new_y)),(0,0,255),12)
                            #         with open('ang_mag.txt','a') as f:
                            #             f.write('{},{},{}\n'.format(len(img_ls),ang,mag))
                            #     except: pass

                            if len(input_data) >=2 and names[int(cls)] == 'person' :
                                vector_result, P1, P2, center, next_center, next_P2, camera_distance = calc_vector(
                                    np.array(input_data), 6)
                                ...
                                cv2.putText(im0,'distance : %.2f'%camera_distance[idx],(130,180),cv2.FONT_HERSHEY_COMPLEX, 1.0, (0, 255, 255), 3)
                                arrow_px = int(center[idx][0]) + 50 * math.cos(
                                    math.radians(vector_result[idx - 1][1]))
                                arrow_py = int(center[idx][1]) + 50 * math.sin(
                                    math.radians(vector_result[idx - 1][1]))
                                cv2.arrowedLine(im0, (int(center[idx][0]), int(center[idx][1])),
                                                (int(arrow_px), int(arrow_py)),
